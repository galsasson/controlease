...
void Div::initInterface(Vec2f size)
{
...
void Div::draw()
{
    ...
    // draw title
    ResourceManager::getInstance().getTextureFont()->drawString("Divide", titleRect);
...
void Div::translate(Vec2f offset)
{
    canvasRect += offset;
}
...
void Div::mouseDown(cease::MouseEvent event)
{
    compDragAnchor = event.getPos();
}

void Div::mouseDrag(cease::MouseEvent event)
{
...
void Div::mouseUp( cease::MouseEvent event)
{
}

void Div::mouseWheel( cease::MouseEvent event ) {}
void Div::mouseMove( cease::MouseEvent event ) {}

bool Div::isDragPoint(cease::MouseEvent event)
{
...
bool Div::isHotspot(cease::MouseEvent event)
{
    Vec2f local = getLocalCoords(event.getPos());
...
ConnectionResult* Div::getConnectionStart(cease::MouseEvent event)
{
    Vec2f local = getLocalCoords(event.getPos());
...
ConnectionResult* Div::getConnectionEnd(cease::MouseEvent event)
{
    Vec2f local = getLocalCoords(event.getPos());
...
vector<Node*> Div::getOutputNodes()
{
    vector<Node*> outputs;
...
Vec2f Div::getCanvasPos()
{
    return canvasRect.getUpperLeft();
}

bool Div::contains(Vec2f p)
{
    return canvasRect.contains(p);
}

float Div::getValue(int i)
{
    return val;
}

void Div::setValue(int i, float v)
{
    vals[i] = v;
    nextVal = vals[0];
...
Vec2f Div::getLocalCoords(Vec2f p)
{
    return p-canvasRect.getUpperLeft();
}

Vec2f Div::getCanvasCoords(Vec2f p)
{
    return canvasRect.getUpperLeft() + p;
}

void Div::updateVal(float newVal)
{
    val = newVal;
